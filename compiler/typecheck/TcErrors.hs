...
instance Ord HoleFit where
  compare a b = compare (hfId a) (hfId b)
...
validSubstitutions simples (CEC {cec_encl = implics}) ct | isExprHoleCt ct =
  do { rdr_env <- getGlobalRdrEnv
     ...
     ; if any (== 0) maxSubs
       then return empty
               ...
               ; (discards, sortedSubs) <-
                   if sortSubs
                   then possiblyDiscard maxSubs <$> sortByGraph subs
                   else return (discards, localsFirst subs)
               ; traceTc "}" empty
  ...
  where
    ...
    possiblyDiscard (Just max) fits = (length fits > max, take max fits)
