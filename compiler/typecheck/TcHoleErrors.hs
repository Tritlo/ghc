...
data HoleFitCandidate =
    IdHFCand Id                -- An id, like locals.
  | NameHFCand Name            -- A name, like built-in syntax.
  | GreHFCand GlobalRdrElt     -- A global, like imported ids.
  | RawHFCand Name SDoc TcType -- A raw candidate, which is just checked as is.
                               -- For plugins to use to check arbitrary
                               -- candidates.  Note that the name here must be
                               -- generated by the plugin.

instance Eq HoleFitCandidate where
  (IdHFCand a) == (IdHFCand b) = (a == b)
  (NameHFCand a) == (NameHFCand b) = (a == b)
  (GreHFCand a) == (GreHFCand b) = (a == b)
  (RawHFCand a _ _) == (RawHFCand b _ _) = (a == b)
  _ == _ = False
...
pprHoleFitCand (RawHFCand name doc ty) =
  text "Raw HFC: " <> ppr name <+> dcolon <+> pprParendType ty <> doc
...
data HoleFit =
  HoleFit { hfId   :: Id -- The elements id in the TcM
          ...
          , hfDoc :: Maybe HsDocString } -- Documentation of this HoleFit, if
                                         -- available.
 -- ^ A regular HoleFit, as generated by valid hole fits.
 | RawHoleFit { hfWrap :: [TcType]  -- The wrapper for the match.
              , rawCand :: Maybe HoleFitCandidate -- The raw candidate checked,
                                                  -- if available.
              , hfOut :: SDoc -- The documentation to be outputted
              }
 -- ^ A fit that is just displayed as is. This way, HoleFitPlugins can inject
 -- any fit they want.
...
instance Ord HoleFit where
  compare (RawHoleFit {hfWrap = wrp_a}) (RawHoleFit {hfWrap = wrp_b})
    = compare (sizeTypes wrp_a) (sizeTypes wrp_b)
  compare (RawHoleFit {}) _ = LT
  compare _ (RawHoleFit {}) = GT
...
pprHoleFit _ hf@(RawHoleFit {}) = hfOut hf
      ...
      where zonkSubs' zs env [] = return (env, reverse zs)
            ...
            zonkSub env hf@HoleFit{hfType = ty, hfMatches = m, hfWrap = wrp}
              = do { (env, ty') <- zonkTidyTcType env ty
                   ...
                   ; return (env, zFit) }
            zonkSub env hf@(RawHoleFit {hfWrap = wrp}) =
                do { (env, wrp') <- zonkTidyTcTypes env wrp
                   ; let zFit = hf {hfWrap = wrp'}
                   ; return (env, zFit) }
  ...
  where
    ...
    go subs seen maxleft ty (el:elts) =
        ...
        do { traceTc "lookingUp" $ ppr el
           ; case el of
               RawHFCand _ doc cand_ty ->
                do { fits <- fitsHole ty cand_ty
                   ; case fits of
                       Just (wrp, _) -> keep_raw doc wrp el
                       _      -> discard_it }
               _ ->
                do { maybeThing <- lookup el
                   ; case maybeThing of
        ...
        where
          ...
          lookup hfc = do { thing <- tcLookup name
                          ; return $ case thing of
                                       ...
                                       _ -> Nothing }
            where name = hfCandName hfc
          discard_it = go subs seen maxleft ty elts
          keep_raw doc wrp el = go ((RawHoleFit { hfWrap = wrp
                                                , rawCand = Just el
                                                , hfOut = doc}):subs)
                               seen ((\n -> n - 1) <$> maxleft) ty elts
